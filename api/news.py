...
def send_news():
    ...
    try:
        contents = get_news()
        send_chatrooms_msg(names,'[每日一句又来啦~]\n%s\n%s' % (contents[0],contents[1]))
    except:
...
def schedule_weather():
    ...
    try:
        ...
        send_chatrooms_msg(names,contents)
    except:
...
def send_chatrooms_msg(senders,context):
    sender = senders[1]
    ...
    wxapi.logger.info("定时任务[%s],发送时间：[%s], 发送到: [%s], 发送内容：[%s]" ,'每日一句',datetime.datetime.now(), sender ,context)
